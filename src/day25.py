...
def build_snafu(target, so_far, idx):
    if target == 0:
        return so_far
    ...
    for digit in (-2, -1, 0, 1, 2):
        ...
        if ((target-value) % (5**(idx+1))) == 0:
            return build_snafu(target-value, digit_to_char[digit] + so_far, idx+1)

def to_snafu(n):
    return build_snafu(n, '', 0)
...
def calculate_fuel(data):
    total = sum(to_decimal(snafu) for snafu in data)
...
def test_calculate_fuel():
    ...
    assert calculate_fuel(data) == '2=-1=0'
