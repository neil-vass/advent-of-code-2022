...
def mix_step(data, idx_in_data, remix):
    ...
    if new_pos > current_pos:
        del remix[current_pos]
    else:
        del remix[current_pos+1]
...
def basic_decrypt_data_file(data, steps=None):
    if steps is None:
        steps = len(data)
...
def test_decrypt_data_step_by_step():
    data = fetch_data('sample_data/day20.txt')
    assert basic_decrypt_data_file(data, steps=1) == [2, 1, -3, 3, -2, 0, 4]
    assert basic_decrypt_data_file(data, steps=2) == [1, -3, 2, 3, -2, 0, 4]
    assert basic_decrypt_data_file(data, steps=3) == [1, 2, 3, -2, -3, 0, 4]
    assert basic_decrypt_data_file(data, steps=4) == [1, 2, -2, -3, 0, 3, 4]
    assert basic_decrypt_data_file(data, steps=5) == [1, 2, -3, 0, 3, 4, -2]
...
def test_basic_decryption():
     assert basic_decrypt_data_file([1,0,0]) == [0,1,0]
     assert basic_decrypt_data_file([3,0,0]) == [0,3,0]
     assert basic_decrypt_data_file([-1,0,0]) == [0,-1,0]
...
if __name__ == "__main__":
    decrypted = decrypt_data_file('data/day20.txt')
